...
//									$("#checkValue").remove()
		...
		function removes(id){
			...
			var fileIds = id
			$.ajax({
						url:'<g:createLink controller="resourceInfo" action="remove" />',
						type:'post',
						dataType:'json',
						data:{fileId:fileIds},
						success:function(msg){
		...
		<div class="left-bar">
			<div class="nav-left">
				<ul class="list-unstyled nav-left-ul">
					...
					<g:link action="categoryPage" id="activeFile" style="text-decoration: none"><li class="current"><i class="fa fa-file-o"></i><span class="text">常用文件</span></li></g:link>
				...
				<div class="all-file tab" style="display:block">
					<div class="header" style="border:none;">
						<g:form controller="resourceInfo" action="categorySearch" method="get" class="navbar-form navbar-right hidden-xs sea" role="search">
							<div class="form-group">
								<input type="text" class="form-control search" placeholder="搜索常用文件" name="search"  value="${search}">
					...
					<div class="body">
						...
						<div class="body-nav">
							...
							<div class="activescrollbar">
								%{--触发滚动事件的条件--}%

								<div class="activefilescrollbar">
								%{--重命名input--}%
...
%{--<div id="b_${resourceInfoInstance.id}">--}%
								<div class="user-file" id="s_${resourceInfoInstance.id}">
									...
									<div class="col user-filename">
										...
										<i class="anniu fa fa-trash" data-toggle="tooltip" data-placement="left" title="移出常用文件" onclick="removes('${resourceInfoInstance.id}')"></i>
...
//		alert(fillType)
	...
	$(function() {
		$(".activescrollbar").scroll(function() {
//			当前页码
			var page = $("#pageNo").val();
							...
							success:function(msg){
								...
								for(var i= 0;i<ResourceInfoList.length;i++){
									...
									var res='<div class="'+resourceInfo.id+'"><div>' +
											...
											'<span style=""></span><span class="file-name">' +
											showDetail +
											'<i class="anniu fa fa-trash" data-toggle="tooltip" data-placement="left" title="移出常用文件" onclick="removes('+"'"+resourceInfo.id+"'"+')"></i>' +
