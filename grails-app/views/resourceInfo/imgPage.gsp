...
<head>
    ...
    <link rel="stylesheet" href="${resource(dir: 'css',file: 'viewer.min.css')}" type="text/css">
    <g:javascript src="viewer.min.js"/>
        ...
        <div class="left-bar">
            <div class="nav-left">
                <ul class="list-unstyled nav-left-ul">
                    ...
                    <g:link action="categoryPage" id="images" style="text-decoration: none"><li class="current"><i class="fa fa-file-image-o"></i><span class="text">图片</span></li></g:link>
                    ...
                    <div class="header" style="border:none;">
                        <g:form controller="resourceInfo" action="categorySearch" method="get" class="navbar-form navbar-right hidden-xs sea" role="search">
                            <div class="form-group">
                                <input type="text" class="form-control search" placeholder="搜索图片" name="search"  value="${search}">
                    ...
                    <div class="body">
                        ...
                        <div class="body-nav">
                            <ul class=" list-unstyled list-col">
                                <li class="first" style="border-right:none;">
                                    <input type="checkbox" id="selAll" onClick="selectAll()">
                                    <span class="file-name" style="margin-left:5px;"></span>
                            ...
                            <div class="imgscrollbar">
                            <div class="imgfilescrollbar">
                                %{--触发滚动事件的条件--}%
                                <div class="imgliscrbar" id="jq">
                                    <g:each in="${ResourceInfoList}" status="i" var="resourceInfoInstance">
                                     <div class="${resourceInfoInstance.id}" >
                                     <div class=" imgspc"  >
                                        <input name="checkAll" type="checkbox" class="imgchek" value="${resourceInfoInstance.id}" onClick="setSelectAll()" />
                                             <div class="thumbnail imglol" >
                                                <img  data-original="${resource(dir:'',file: resourceInfoInstance.path)}" src="${resource(dir: '',file: resourceInfoInstance.preImgPath)}" class="special_img" >
                                                 <div class="ct">
                                                       <a class="imgtitle">图片名称：${raw(resourceInfoInstance.name)}</a>
                                                     <div class="bottom_dow">
                                                         <a><i class="anniu fa fa-download" title="下载" onclick="singleFileDownload('${resourceInfoInstance.id}')"></i></a>
                                                         <a><i class="anniu fa fa-arrow-left" title="添加至常用" onclick="moveCommon('${resourceInfoInstance.id}')"></i></a>
                                                         <a><i class="anniu fa fa-chain gl" title="查看关联" onclick="showLink('${resourceInfoInstance.id}')"></i></a>
                                                     </div>
                                                 </div>
                                            </div>
                                    </div>
...
//		alert(fillType)
    ...
    var viewer = new Viewer(document.getElementById('jq'), {
        url: 'data-original'
...
//    }
    $(function() {
        ...
        $(".imgscrollbar").scroll(function() {
                ...
                $.ajax({
                    ...
                    success:function(msg){
                        ...
                        for(var i= 0;i<ResourceInfoList.length;i++){
                            ...
                            var res='<div class="'+resourceInfo.id+'">' +
                                    '<div class="imgspc">' +
                                    '<input type="checkbox" class="imgchek" name="checkAll" value="'+resourceInfo.id+'" onClick="setSelectAll()">' +
                                    '<div class="thumbnail imglol" >' +
                                    '<img data-original="'+path+resourceInfo.path+'" onclick="" src="'+path+resourceInfo.preImgPath+'" class="special_img">' +
                                    '<div class="ct">' +
                                    '<a class="imgtitle">图片名称：'+resourceInfo.name+'</a><div class="bottom_dow">' +
                                    '<a><i class="anniu fa fa-download" title="下载" onclick="singleFileDownload('+"'"+resourceInfo.id+"'"+')"></i></a>' +
                                    '<a><i class="anniu fa fa-arrow-left" title="添加至常用" onclick="moveCommon('+"'"+resourceInfo.id+"'"+')"></i></a>' +
                                    '<a><i class="anniu fa fa-chain gl" title="查看关联" onclick="showLink('+"'"+resourceInfo.id+"'"+')"></i></a>' +
                                    '</div></div></div></div></div>'
            ...
            viewer = new Viewer(document.getElementById('jq'), {
                url: 'data-original'
            });
        })
